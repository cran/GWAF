...
setMethod('sum', signature('bdsmatrix'),
	  function(x, ..., na.rm=F) {
	      ...
	      temp <- .C('bdsmatrix_index1',
			 ...
			 integer(1),
			 indexb = integer(d3),
			 integer(1), PACKAGE="GWAF")$indexb     #index of diagonal elements
...
bdsmatrix <- function(blocksize, blocks, rmat, dimnames=NULL) {
	...
	#  half
        temp <- .C("bdsmatrix_index3",
		   ...
		   index=integer(n3), PACKAGE="GWAF")$index
	blocks <- blocks[temp]
	}
...
#   cols <- (1:d[2])[..2]
	...
	temp <- .C('bdsmatrix_index1',
		   ...
		   integer(1),
		   integer(1),
		   indexc = integer(d4), PACKAGE="GWAF")
	...
	if (brlen>0 && bclen>0) {
	    ...
	    temp <- .C('bdsmatrix_index2',
		       ...
		       cols= integer(length(x@blocks)),PACKAGE="GWAF")
