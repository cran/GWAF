     ...
     for (i in snplist) {
          ...
          if (bin.flag>2) {lme.out<-try(lmekin(test2.dat[,phen]~snp+x.int+x.covar,random=~1|id,varlist=kmat,na.action=na.omit)) 
             if (!class(lme.out)%in%"try-error"){
                tmp<-c(lme.out$var["snp","x.int"],mod.lab,lme.out$ctable["snp",1],sqrt(lme.out$var["snp","snp"]),pchisq(lme.out$ctable["snp",1]^2/lme.out$var["snp","snp"],1,lower.tail=F),
                     ...
                     ### add cov_beta_snp_beta_int 091409   
          } else { ###bin.flag==2          
          if (sub=="Y") {
             if (sum(table(test2.dat[,cov.int],snp)[1,-1])==0 | sum(table(test2.dat[,cov.int],snp)[2,-1])==0) tmp<-rep(NA,13) else {
                ...
                if (!class(lme.out)%in%"try-error" & !class(lme.out1)%in%"try-error" & !class(lme.out2)%in%"try-error"){
                   tmp<-c(mod.lab,lme.out$ctable["snp",1],sqrt(lme.out$var["snp","snp"]),pchisq(lme.out$ctable["snp",1]^2/lme.out$var["snp","snp"],1,lower.tail=F),
